...
def x_softmax(logits, dim=-1, temperature=1.0, noise_source=None):
    if noise_source == "gumbel":
        return nn.functional.gumbel_softmax(logits, tau=temperature, hard=False)
    else:
        logits_ = logits
    return nn.functional.softmax(logits_ / temperature, dim=dim)
...
class L2Normalize(nn.Module):
    ...
    def forward(self, inputs, p=2, dim=-1):
        return nn.functional.normalize(inputs, p=p, dim=dim)
...
class Controller(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.is_norm_fc_module = kwargs['is_norm_fc_module']
        ...
        self.fc_q_cat_c = nn.Linear(2*self.dim_lstm, self.dim_lstm)
        ...
        if self.is_norm_fc_module:
            fc_module_weight_.append(L2Normalize())
        self.fc_module_weight = nn.Sequential(*fc_module_weight_)
    ...
    def forward(self, lstm_seq, q_encoding, embed_seq, seq_length_batch):
        ...
        for t in range(self.T_ctrl):
            ...
            if self.use_gumbel:
                ...
                noise_source = 'softmax'

            module_prob = x_softmax(module_logit, temperature=self.temperature, noise_source=noise_source)
